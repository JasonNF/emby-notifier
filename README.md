...
📺 一个为 Emby 服务器的管理员打造的全功能 Telegram Bot，支持媒体事件推送、系统事件告警、交互式搜索、播放状态监控、网盘文件管理、Emby用户管理、精细化权限与展示设置，并具备自动消息删除功能。适用于家庭服务器、媒体分享群组等多种场景，支持 Docker 快速部署。
...
- 🗃️ **模块化远程管理**：`/manage` 命令现已分为“节目与文件管理”和“用户管理”两大模块，支持远程更新、入库、删除媒体文件（视频文件以.strm方式）。
...
- 🌐 **智能信息展示**：音轨语言自动翻译为中文，字幕轨道按常用语言优先排序，IP 地理归属地信息清晰展示。
...
| `/manage` | 管理媒体文件与 Emby 用户，支持更新、入库、删除等高级操作。 | 管理员 | 👉 [/manage.md](https://github.com/xpisce/emby-notifier/blob/main/User%20Guide/userguide-manage.md) |
...
| 字幕规格信息将中文字幕展示在最前面，字幕较多时只展示部分 | ✅ 已完成（v2.4.4） |
...
| 给/manage命令增加创建/删除/修改Emby用户的功能 | ✅ 已完成（v2.4.4） |
