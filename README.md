...
📺 一个为 Emby 服务器的管理员打造的全功能 Telegram Bot，支持媒体事件推送、交互式搜索、播放状态监控、远程文件管理、精细化权限与展示设置，并具备自动消息删除功能。适用于家庭服务器、媒体分享群组等多种场景，支持 Docker 快速部署。
...
- 🗃️ **远程文件管理**：支持通过 `/manage` 命令远程更新、入库、删除媒体文件，支持云端（`.strm`）与本地文件操作。
...
| 媒体文件管理 | `/manage` 支持从云端更新、添加入库、删除 Emby 条目与物理文件。 |
